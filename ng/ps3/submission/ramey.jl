...
b025, t975 = runkle(dataSamp1, Aols1, μols1, resids1, length(μols1), 12, 499, 48,recursiveID=true,Clevel=90)

logVars=[1,3,4,6,7,8]
logVarsSmall=[1,3,4]
irf1[logVars,:] = 100*irf1[logVars,:]
irf2[logVars,:] = 100*irf2[logVars,:]
irf3[logVarsSmall,:] = 100*irf3[logVarsSmall,:]

b025[:,logVars,:] = 100*b025[:,logVars,:]
t975[:,logVars,:] = 100*t975[:,logVars,:]

VNAMES = ["IP", "UR", "CPI", "COMMODITY", "FF", "NBR", "TR", "M1"]
for ii = 1:length(VNAMES)
    if ii <= 5
        plotIRF([irf1[ii,:] irf2[ii,:]  irf3[ii,:]], ["CEE Dates", "Update", "Small VAR"],
                string("cee_" ,VNAMES[ii]),
                CIlow = b025[:,ii,5], CIhigh=t975[:,ii,5],TITLE=VNAMES[ii],Clevel=90)
    else
        plotIRF([irf1[ii,:] irf2[ii,:]], ["CEE Dates", "Update", "Small VAR"], string("cee_" ,VNAMES[ii]),
                CIlow = b025[:,ii,5], CIhigh=t975[:,ii,5],TITLE=VNAMES[ii],Clevel=90)
    end
end
